...
N_tp    = 1024;                % 慢时间点数 % 64 % 256 % 2048 % 512 % 1024

% Adjustable
ang_res = 1;                   % Resolution/Grain of angle  5    1
L_win_AoA = 5;                 % Length of searching window for Rx-AoA estimation and tracking  5   10
L_win_AoD = 5;                 % Length of searching window for Tx-AoD estimation and tracking  5   10
h_eq_tilde_th = 0.5;           % Do beamsweeping as the calibration when 'abs(h_eq_tilde) < h_eq_tilde_th'  0.8 0.5
if if_steer_vec_norm == 0
    h_eq_tilde_th = 10.* h_eq_tilde_th;
end
set_target_velocity = 0.3;        % m/s   2   5   5   1   0.2   0.3 0.4 0.5   1 1.5-far 0.3-near
...
Tar_1.sigma = 0.5;                          % 0.5
Tar_1.sigma_v = set_target_velocity;        % m/s   2   5   5   1   0.2   0.3 0.4 0.5   1 1.5 0.3
...
for it = 1:N_tp
    for ntt = 1:Tx_1.N
        ...
        if if_doppler_effect == 1 % Considering Doppler effect
            V_nr_nt__l(:,ntt,it) = Uplink_1_1.Velocity(it,:).'+Downlink_1_1.Velocity(it,ntt);
        end
    end
end
% ????? TEMP TEST (Should be commented) ?????
% for it = 1:N_tp
%     temp_V_nr_nt = V_nr_nt__l(1,1,it);
%     for ntt = 1:Tx_1.N
%         for nrr = 1:Rx_1.N
%             V_nr_nt__l(nrr,ntt,it) = temp_V_nr_nt;
%         end
%     end
% end
% ????? TEMP TEST (Should be commented) ?????
...
% w_test = randi(9,1,4);
% f_test = randi(9,8,1);
% (w_test*(A_test.*B_test)*f_test)/(w_test*B_test*f_test)
% (w_test*A_test*f_test)

% A_test = rand(Rx_1.N,Tx_1.N)+jmath.*rand(Rx_1.N,Tx_1.N);
% B_test = rand(Rx_1.N,Tx_1.N)+jmath.*rand(Rx_1.N,Tx_1.N);
% w_test = Rx_1.DFT_codebook(:,5);
% f_test = Tx_1.DFT_codebook(:,10);
% (w_test'*(A_test.*B_test)*f_test)/(w_test'*B_test*f_test)
% (w_test'*A_test*f_test)
...
rec_period_cali_beam_sweeping( 1 ,1) = 1; % special in AoD tracking process
if if_steer_vec_norm == 1
    for l = 1:N_tp
        for k = 1:N_sc
            if Rx_Tx_turn_indicator == 1 % --------Rx
                ...
                if count_win_AoA == L_win_AoA % Potential error revised
                    center_number_win_AoA = temp_number_AoA__maximum_h_eq_tilde;
                    % if (if_beemsweeping_cali == 1) && (temp_maximum_h_eq_tilde < h_eq_tilde_th) % Lost Target calibration
                    %     % Calibrating AoA and AoD
                ...
                if count_win_AoD == L_win_AoD % Potential error revised
                    center_number_win_AoD = temp_number_AoD__maximum_h_eq_tilde;
                    if (if_beamsweeping_cali == 1) && (temp_maximum_h_eq_tilde < h_eq_tilde_th) % Lost Target calibration   % special in AoD tracking process
                        % Calibrating AoA and AoD
                        center_number_win_AoA = ang2beam_number(ang_array_deg,Rx_1.ang_real_array_deg(l,1)); % special in AoD tracking process
                        center_number_win_AoD = ang2beam_number(ang_array_deg,Tx_1.ang_real_array_deg(l,1));
                        % Rx-BS
                        w = Rx_1.DFT_codebook(:,center_number_win_AoA)./sqrt(Rx_1.N);
                        % Tx-UE
                        ...
                        h_eq_tilde_k_l(k,l) = w'*H_tilde_nr_nt__k_l(:,:,k,l)*f;
                        h_eq_tilde_bir_bit__k_l(search_number_win_AoA,center_number_win_AoD,k,l) = h_eq_tilde_k_l(k,l);
                        temp_maximum_h_eq_tilde = abs(h_eq_tilde_k_l(k,l)); % special in AoD tracking process
                        count_cali_beam_sweeping = count_cali_beam_sweeping + 1;
                        rec_l_cali_beam_sweeping(count_cali_beam_sweeping,1) = l;
                        rec_period_cali_beam_sweeping(count_cali_beam_sweeping,1) = ceil(l/(L_win_AoA+L_win_AoD)); % special in AoD tracking process
                        rec_center_number_win_AoA(ceil(l/(L_win_AoA+L_win_AoD)),1) = center_number_win_AoA;
                    end
                    rec_center_number_win_AoD(ceil(l/(L_win_AoA+L_win_AoD)),1) = center_number_win_AoD;
                    rec_channel_gain_max_period(ceil(l/(L_win_AoA+L_win_AoD)),1) = temp_maximum_h_eq_tilde; % special in AoD tracking process
                    temp_maximum_h_eq_tilde = 0;
                    count_win_AoD = 0;
                    Rx_Tx_turn_indicator = (-1)*Rx_Tx_turn_indicator;
                end
...
for i = 1:size(Tar_1.trajectory_array_est,1)-1
    % color_string = strcat('#',dec2hex(round(i/(size(Tar_1.trajectory_array_est,1)-1)*16777215/2+16777215/2)));
    color_string = 'red';
    % text(Tar_1.trajectory_array(i*(L_win_AoA+L_win_AoD),1), Tar_1.trajectory_array(i*(L_win_AoA+L_win_AoD),2)+0.002, num2str(i),'Color',color_string,'FontSize',9); % Mark the serial numbers and adjust the positions slightly to avoid overlap
    text(Tar_1.trajectory_array(i*(L_win_AoA+L_win_AoD),1), Tar_1.trajectory_array(i*(L_win_AoA+L_win_AoD),2), num2str(i),'Color',color_string,'FontSize',9); % Mark the serial numbers and adjust the positions slightly to avoid overlap
end
plot(Tar_1.trajectory_array_est(:,1),Tar_1.trajectory_array_est(:,2),'b-sq');hold on;
for i = 1:size(Tar_1.trajectory_array_est,1) % ***** SEPARATE AND PAUSE HERE *****
    color_string = strcat('#',dec2hex(round(i/size(Tar_1.trajectory_array_est,1)*16777215/2+16777215/2)));
    % color_string = strcat('#','0F0F0F');
    % text(Tar_1.trajectory_array_est(i,1)-0.0012, Tar_1.trajectory_array_est(i,2)+0.005, num2str(i),'Color',color_string,'FontSize',9); % Mark the serial numbers and adjust the positions slightly to avoid overlap
    text(Tar_1.trajectory_array_est(i,1), Tar_1.trajectory_array_est(i,2), num2str(i),'Color',color_string,'FontSize',9); % Mark the serial numbers and adjust the positions slightly to avoid overlap
end
legend('Target starting position','True trajectory','Estimated trajectory');hold on;
xlabel('x [m]');hold on;
ylabel('y [m]');hold on;
...
if if_beamsweeping_cali == 1
    ...
    fprintf("Slot index of beamsweeping calibration:\n");
    vector_temp = rec_l_cali_beam_sweeping.';
    fprintf("%d ",vector_temp);
    fprintf("\n");
    fprintf("Period index of beamsweeping calibration:\n");
    vector_temp = rec_period_cali_beam_sweeping.';
    fprintf("%d ",vector_temp);
    fprintf("\n");
end
...
% Simulaton settings output
fprintf("Field type: %s,  ",select_field_type);

fprintf("Target velocity set as: %d,  ",set_target_velocity);

if if_beamsweeping_cali == 0
    fprintf("Beamsweeping calibration OFF,  ");
else
    fprintf("Beamsweeping calibration ON,  ");
end

if if_doppler_effect == 0
    fprintf("Doppler effect OFF,  ");
else
    fprintf("Doppler effect ON,  ");
end

if if_path_loss == 0
    fprintf("Path loss OFF,  ");
else
    fprintf("Path loss ON,  ");
    fprintf("Path loss type: %s,  ",select_path_loss_type);
end

if if_steer_vec_norm == 0
    fprintf("Steering vector normalization OFF,  ");
else
    fprintf("Steering vector normalization ON,  ");
end
